...
RSpec.describe SponsorVisitToken do
  describe ".generate" do
    it "generates a stable opaque token for an event and sponsor" do
      token = described_class.generate(event_slug: "2026", sponsor_key: "smartbank.co.jp")

      expect(token).to eq(described_class.generate(event_slug: "2026", sponsor_key: "smartbank.co.jp"))
      expect(token).not_to include("2026", "smartbank.co.jp")
      expect(token.length).to eq(43)
    end

    it "generates different tokens for different events" do
      token_2025 = described_class.generate(event_slug: "2025", sponsor_key: "smartbank.co.jp")
      token_2026 = described_class.generate(event_slug: "2026", sponsor_key: "smartbank.co.jp")
    ...
    it "generates different tokens when the secret changes" do
      ...
      expect(described_class.generate(event_slug: "2026", sponsor_key: "smartbank.co.jp")).not_to eq(token)
    end
  end
  ...
  describe ".stamp_url" do
    it "uses the configured origin, port and path prefix" do
      ...
      expect(described_class.stamp_url(event_slug: "2026", sponsor_key: "smartbank.co.jp")).to eq(
  ...
  describe ".find_sponsor" do
    let(:sponsors) do
      [
        {key: "smartbank.co.jp", name: "SmartBank, Inc.", plan: "gold"},
        {key: "coincheck.com", name: "Coincheck, Inc.", plan: "silver"}
      ]
    end
    ...
    it "does not accept an invalid or differently scoped token" do
      token = described_class.generate(event_slug: "2025", sponsor_key: "smartbank.co.jp")
